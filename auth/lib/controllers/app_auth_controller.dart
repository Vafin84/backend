...
class AppAuthController extends ResourceController {
  ...
  Future<Response> signIn(@Bind.body() User user) async {
    ...
    try {
      ...
      final requestHasPassword = AuthUtility.generatePasswordHash(user.password ?? "", findUser.salt ?? "");
      ...
      if (requestHasPassword == findUser.hashPassword) {
        ...
        return Response.ok(ResponseModel(data: newUser?.backing.contents, message: "Успешная авторизация"));
      } else {
  ...
  Future<Response> signUp(@Bind.body() User user) async {
    ...
    try {
      ...
      return Response.ok(ResponseModel(data: userData?.backing.contents, message: "Успешная регистрация"));
    } on QueryException catch (error) {
      return Response.serverError(body: ResponseModel(message: error.message));
    }
  }
  ...
  Future<Response> refreshToken(@Bind.path("refresh") String refreshToken) async {
    try {
      ...
      if (user?.refreshToken != refreshToken) {
        return Response.unauthorized(body: ResponseModel(message: "Token is not valid"));
      } else {
        await _updateTokens(id, managedContext);
        final user = await managedContext.fetchObjectWithID<User>(id);
        return Response.ok(ResponseModel(data: user?.backing.contents, message: "Успешное обновление токенов"));
      }
    } catch (error) {
      return Response.serverError(body: ResponseModel(message: error.toString()));
    }
  }
