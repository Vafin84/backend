...
class AppAuthController extends ResourceController {
  ...
  Future<Response> signIn(@Bind.body() User user) async {
    ...
    try {
      ...
      if (requestHasPassword == findUser.hashPassword) {
        ...
        return AppResponse.ok(body: newUser?.backing.contents, message: "Успешная авторизация");
      } else {
        throw QueryException.input("Пароль неверный", []);
      }
    } catch (error) {
      return AppResponse.serverError(error, message: "Ошибка автроизации");
    }
  }
  ...
  Future<Response> signUp(@Bind.body() User user) async {
    ...
    try {
      ...
      return AppResponse.ok(body: userData?.backing.contents, message: "Успешная регистрация");
    } catch (error) {
  ...
  Future<Response> refreshToken(@Bind.path("refresh") String refreshToken) async {
    try {
      ...
      if (user?.refreshToken != refreshToken) {
        ...
        final user = await managedContext.fetchObjectWithID<User>(id);
        return AppResponse.ok(body: user?.backing.contents, message: "Успешное обновление токенов");
      }
    } catch (error) {
